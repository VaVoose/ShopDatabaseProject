...
IMPORTANT NOTE:
This project assumes that student AND faculty ID numbers follow a pattern. Found on the ONU ID card is a a ID number
This number listed is 11 characters long. The usuable part of the ID is the first 9 characters of the ID, the last
2 characters are reserved for if a student looses their ID and gets another with those 2 characters being incremented 
by 1. Also it assumes that the ID scanned from the card contains the student ID with a ';' character prededing it and 
a '?' character at the end of it. If either of the 2 characters change from swipe scanner to swipe scanner the program
